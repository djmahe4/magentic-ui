...
async def get_task_team(
    ...
    *,
    ...
    """
    if magentic_red_ui_config is None:
    ...
    approval_policy = (
        magentic_red_ui_config.approval_policy
        if magentic_red_ui_config.approval_policy
        else "never"
    )
    ...
    browser_resource_config, _novnc_port, _playwright_port = (
        get_browser_resource_config(
            ...
            magentic_red_ui_config.novnc_port,
    ...
    orchestrator_config = OrchestratorConfig(
        cooperative_planning=magentic_red_ui_config.cooperative_planning,
        autonomous_execution=magentic_red_ui_config.autonomous_execution,
        allowed_websites=magentic_red_ui_config.allowed_websites,
        plan=magentic_red_ui_config.plan,
        model_context_token_limit=magentic_red_ui_config.model_context_token_limit,
        do_bing_search=magentic_red_ui_config.do_bing_search,
        retrieve_relevant_plans=magentic_red_ui_config.retrieve_relevant_plans,
    ...
    websurfer_config = WebSurferConfig(
        ...
        max_actions_per_step=magentic_red_ui_config.max_actions_per_step,
        url_statuses={key: "allowed" for key in orchestrator_config.allowed_websites}
        if orchestrator_config.allowed_websites
        else None,
        url_block_list=get_internal_urls(magentic_red_ui_config.inside_docker, paths),
        multiple_tools_per_call=magentic_red_ui_config.multiple_tools_per_call,
    ...
    if magentic_red_ui_config.user_proxy_type == "dummy":
        ...
        user_proxy = MetadataUserProxy(
            ...
            task=magentic_red_ui_config.task,
            helpful_task_hints=magentic_red_ui_config.hints,
            task_answer=magentic_red_ui_config.answer,
    ...
    if (
        ...
        memory_provider = MemoryControllerProvider(
            ...
            inside_docker=magentic_red_ui_config.inside_docker,
        )
    else:
